...
<body>
    ...
    <div class="container mt-3">
        ...
        $sql = "SELECT DISTINCT id FROM timetable"; 
    ...
    if (isset($_POST['submit']) && ($sel1 || $selectedMonth)) {
        ...
        $query = "SELECT * FROM timetable LIMIT ?, ?";
        $stmt = $conn->prepare($query);
    ...
    $total_rows_query = "SELECT COUNT(*) AS total FROM timetable";
    ...
    if ($result_teacher->num_rows > 0) {
        ...
        while ($row_teacher = $result_teacher->fetch_assoc()) {
            ...
            echo "<td>" . htmlspecialchars($row_teacher['time_range']) . "</td>";
            echo "<td>" . htmlspecialchars($row_teacher['subject_name']) . "</td>";
       
            echo "<td>";
            echo "<a href='edit_lecture.php?lecture_id=" . htmlspecialchars($row_teacher['lecture_id']) . "' class='btn btn-warning btn-sm me-2'><i class='bi bi-pencil'></i> 変更</a>";
            echo "<a href='delete_lecture.php?lecture_id=" . htmlspecialchars($row_teacher['lecture_id']) . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this lecture?\");'><i class='bi bi-trash'></i> 削除</a>";
            echo "</td>";
            echo "</tr>";
        }
        echo "</tbody></table>";
